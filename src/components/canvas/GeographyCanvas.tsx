  ...
  const runSynthesize = useCallback(() => {
    ...
    void (async () => {
      try {
        ...
        // ── Semantic scene sketch ──────────────────────────────────────────
        // Generate a rough painting-style sketch where each pin is drawn as a
        // recognisable silhouette icon at its exact screen position.  Flux was
        // trained on rough-sketch → photorealistic conversions, so an input
        // that actually looks like a rough scene (sky/ground + iconic shapes)
        // at strength=0.82 gives it the spatial structure it needs to place
        // objects correctly while fully transforming the visual style.
        ...
        if (!sceneryImageUrl) {
          try {
            ...
            if (ctx) {
              // Sky gradient (top 45%)
              const sky = ctx.createLinearGradient(0, 0, 0, size.height * 0.45);
              sky.addColorStop(0, "#87CEEB");
              sky.addColorStop(1, "#C9E9F6");
              ctx.fillStyle = sky;
              ctx.fillRect(0, 0, size.width, size.height * 0.45);
              // Ground gradient (bottom 55%)
              const ground = ctx.createLinearGradient(0, size.height * 0.45, 0, size.height);
              ground.addColorStop(0, "#8FBC6F");
              ground.addColorStop(1, "#5D4037");
              ctx.fillStyle = ground;
              ctx.fillRect(0, size.height * 0.45, size.width, size.height * 0.55);

              pinRefs.forEach((pin) => {
                const { canvas_x: cx, canvas_y: cy } = pin;
                const yNorm = cy / size.height;
                // Perspective scale: objects near the bottom are larger
                const base = size.width * 0.045;
                const s    = base * (0.45 + yNorm * 1.4);
                ...
                if (/\bsun\b/.test(lbl)) {
                  ...
                  ctx.strokeStyle = "#FFA500"; ctx.lineWidth = s * 0.12;
                  for (let i = 0; i < 8; i++) {
                    const a = (i * Math.PI * 2) / 8;
                    ctx.beginPath();
                    ctx.moveTo(cx + Math.cos(a) * s * 1.1, cy + Math.sin(a) * s * 1.1);
                    ctx.lineTo(cx + Math.cos(a) * s * 1.6, cy + Math.sin(a) * s * 1.6);
                    ctx.stroke();
                  }
                } else if (/moon/.test(lbl)) {
                  ctx.fillStyle = "#F5F5DC";
                  ctx.beginPath(); ctx.arc(cx, cy, s * 0.9, 0, Math.PI * 2); ctx.fill();
                } else if (/cloud/.test(lbl)) {
                  ctx.fillStyle = "#FFFFFF";
                  for (const [dx, dy, r] of [[-s*0.5, 0, s*0.55], [s*0.5, 0, s*0.55], [0, -s*0.3, s*0.65]]) {
                    ctx.beginPath(); ctx.arc(cx + dx, cy + dy, r, 0, Math.PI * 2); ctx.fill();
                  }
                } else if (/mountain|hill|peak|cliff|volcano/.test(lbl)) {
                  ctx.fillStyle = "#4A4E69";
                  ctx.beginPath();
                  ctx.moveTo(cx, cy - s * 1.6);
                  ctx.lineTo(cx - s * 1.5, cy + s * 0.9);
                  ctx.lineTo(cx + s * 1.5, cy + s * 0.9);
                  ...
                  ctx.lineTo(cx + s * 0.45, cy - s * 0.85);
                  ctx.closePath(); ctx.fill();
                } else if (/tree|forest|wood/.test(lbl)) {
                  ...
                  ctx.fillRect(cx - s * 0.18, cy + s * 0.5, s * 0.36, s * 0.7);
                } else if (/river|lake|ocean|sea|water|stream|pond/.test(lbl)) {
                  ctx.fillStyle = "#1E90FF";
                  ctx.beginPath();
                  ctx.ellipse(cx, cy, s * 1.5, s * 0.55, 0, 0, Math.PI * 2);
                  ctx.fill();
                  // Shine
                  ctx.fillStyle = "rgba(255,255,255,0.35)";
                  ctx.beginPath();
                  ctx.ellipse(cx - s * 0.3, cy - s * 0.15, s * 0.5, s * 0.15, -0.3, 0, Math.PI * 2);
                  ctx.fill();
                } else if (/village|city|town|building|house|castle/.test(lbl)) {
                  ctx.fillStyle = "#8B7355";
                  for (const [dx, h] of [[-s*0.7, 1.0], [0, 1.4], [s*0.7, 1.0]]) {
                    ctx.fillRect(cx + dx - s * 0.28, cy - s * h, s * 0.56, s * h + s * 0.25);
                    // Roof
                    ctx.fillStyle = "#6B4C2A";
                    ctx.beginPath();
                    ctx.moveTo(cx + dx, cy - s * (h + 0.45));
                    ctx.lineTo(cx + dx - s * 0.38, cy - s * h);
                    ctx.lineTo(cx + dx + s * 0.38, cy - s * h);
                    ctx.closePath(); ctx.fill();
                    ctx.fillStyle = "#8B7355";
                  }
                ...
                ctx.strokeStyle = "rgba(0,0,0,0.85)"; ctx.lineWidth = fs * 0.28;
                ctx.strokeText(pin.label, cx, cy + s * 1.05);
                ctx.fillStyle = "#FFFFFF";
                ctx.fillText(pin.label, cx, cy + s * 1.05);

                ctx.restore();
              });

              layoutDataUrl = offscreen.toDataURL("image/jpeg", 0.90);
            }
          } catch {
            // Best-effort; falls back to text-only generation
          }
        }
